...
/** Pinned scroll UIs (card tracks, the simulator's controls): keep native clicks; shoot handlers must ignore this subtree. */
...
const GunViewer = dynamic(() => import('./GunViewer'), {
  ssr: false,
 ...
 * Avoids matching wrapper divs before real text nodes.
 */
 ...
 */
function findTextOffsetAtPointWithRanges(
  root: HTMLElement,
  x: number,
  y: number
  ...
  return (
    <>
      ...
      {(isOn || splats.length > 0) && (
        <div className="shoot-splat-layer" aria-hidden="true">
          {splats.map((splat) => (
            <div
              key={splat.id}
              className="shoot-splat"
              style={{
                left: splat.x,
                top: splat.y,
                width: splat.size,
      ...
      {isOn && showGunCursor && !hoverShootControls && (
        <div
          ...
          data-shoot-ui="1"
        >
          <span className="shoot-crosshair-v" />
          <span className="shoot-crosshair-h" />
          <span className="shoot-crosshair-dot" />
          <span className="shoot-crosshair-tick" />
        </div>
      )}
      ...
      <div className="shoot-hud" data-shoot-ui="1">
        ...
        >
          ...
          >
            <span className={`shoot-toggle-track${isOn ? ' is-on' : ''}`} aria-hidden="true">
              <span className="shoot-toggle-thumb" />
            </span>
            <span className="shoot-toggle-label">Shoot</span>
          </button>
        </div>

        <div className="shoot-hud-item shoot-hud-clock">
          <span className="shoot-toggle-label">{dateTimeText}</span>
        </div>
      </div>
    </>
  )
